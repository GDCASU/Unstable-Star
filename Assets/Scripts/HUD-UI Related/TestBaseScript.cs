...
public class TestBaseScript : MonoBehaviour
{
  ...
  //  public SoundTag soundTag;
    ...
    void Update()
    {
        ...
        if (/*animator.GetBool("toCSTF")==false*/animator.GetBool("toSIF") == true && Input.GetKeyDown(KeyCode.RightArrow))
        {
        ...
        if (/*happened*/animator.GetBool("toCPTF")==true && (secrtransform.localPosition.z > 59f))
        {
            animator.SetBool("toPIF", true);
            animator.SetBool("toCPTF", false);
            if(happened)
